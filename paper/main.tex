...
\title{[Re] INP-Former: Reconstructing Intrinsic Normal Prototypes for Industrial Anomaly Detection}
...
\begin{itemize}
    ...
    \item \textbf{MVTec AD 2}~\citep{mvtecad2_2024}: 7 categories with more challenging anomalies including subtle defects and lighting variations. Not evaluated in the original paper.
    \item \textbf{Real-IAD}~\citep{realiad2024}: 30 categories of real industrial products. Used as source domain for transfer learning experiments.
\end{itemize}
...
All experiments were conducted on the FRIDA HPC cluster (University of Ljubljana) using NVIDIA A100-SXM4-40GB and L4-24GB GPUs, using a Singularity container with PyTorch and CUDA. Each multi-class training run (200 epochs) takes approximately 7 hours on A100 or 15 hours on L4.
...
Table~\ref{tab:ablation_loss} shows the loss/module ablation with multiple random seeds on MVTec AD, alongside the values reported in the original paper (Table~5 of \citealt{inp_former_2025}). Removing the INP mechanism entirely (``No INP'' row) reduces I-AUROC by 1.4 points and P-AUPRO by 3.3 points compared to INP + $\mathcal{L}_{sm}$, confirming that the INP mechanism provides a meaningful contribution. Our No INP result (98.19) is slightly lower than the original's (98.59), but the gap to INP-enabled configurations is consistent.

\begin{table}[t]
\caption{Loss and module ablation on MVTec AD. ``Original'' values from Table~5 of \citet{inp_former_2025}; ``Ours'' are mean $\pm$ std over 5 seeds. Rows marked --- were not reproduced due to computational constraints. Row 4$^\dagger$ was not tested in the original paper; we add it to isolate $\mathcal{L}_{sm}$'s contribution.}
...
The ablation in Table~\ref{tab:ablation_loss} addresses Claim 3. Our Full configuration achieves $99.62 \pm 0.01$\% I-AUROC and $94.92 \pm 0.07$\% P-AUPRO, closely matching the original (99.67/94.87).

The original paper tested INP + $\mathcal{L}_c$ (without $\mathcal{L}_{sm}$) but never isolated INP + $\mathcal{L}_{sm}$ (without $\mathcal{L}_c$). We fill this gap: our INP + $\mathcal{L}_{sm}$ achieves $99.64 \pm 0.04$\% I-AUROC and $94.85 \pm 0.08$\% P-AUPRO---nearly identical to the Full configuration. This suggests $\mathcal{L}_{sm}$ alone captures most of the benefit, and adding $\mathcal{L}_c$ on top provides negligible improvement ($<$0.1 points).

Comparing to the original paper's results, the picture is more nuanced. Their INP + $\mathcal{L}_c$ configuration achieves 99.61/95.10 (I-AUROC/P-AUPRO), the highest P-AUPRO of any configuration---including Full (94.87). This suggests $\mathcal{L}_c$ may contribute meaningfully to localization, even if $\mathcal{L}_{sm}$ drives most of the detection improvement. However, we were unable to reproduce the INP + $\mathcal{L}_c$ and INP-only configurations to verify this interaction due to computational constraints.

Due to these constraints (each configuration requires $\sim$75 GPU-hours for 5 seeds), we reproduced 3 of the 5 configurations on MVTec AD only, choosing the endpoints (No INP, Full) and one novel intermediate (INP + $\mathcal{L}_{sm}$) to maximize the insight per GPU-hour. The original paper additionally reports results on VisA, where the same trends hold.

Note the substantially lower variance of the INP configurations ($\pm 0.01$--$0.04$) compared to No INP ($\pm 0.65$), suggesting that the INP mechanism also improves training stability---a finding not discussed in the original paper.
...
Interestingly, the optimal $M$ on MVTec AD 2 is lower ($M=2$ for I-AUROC, $M=4$ for P-AUROC) than on MVTec AD/VisA ($M=6$). Performance degrades monotonically for $M \geq 4$ on I-AUROC. This suggests that the ideal number of prototypes is dataset-dependent: MVTec AD 2 has fewer categories (7 vs.\ 15) and different anomaly characteristics, potentially requiring fewer prototypes to capture the normal distribution. With too many prototypes, the model may overfit to noise or redundantly represent similar normal patterns.
...
LoRA finetuning with only 5\% trainable parameters recovers the majority of fully-trained performance. At $r=8$, MVTec AD reaches 98.2\% I-AUROC (vs.\ 99.7\% full train) and VisA reaches 94.4\% (vs.\ 98.9\%). Performance scales consistently with rank: each doubling of $r$ improves I-AUROC by 0.3--1.4 points, with diminishing returns. The improvement over zero-shot transfer is dramatic: +29.0 I-AUROC on MVTec AD and +43.0 on VisA for $r=8$. P-AUPRO shows a similar trend, with LoRA $r=8$ reaching 92.5\% vs.\ 95.0\% full train on MVTec AD. This demonstrates that the decoder representations learned on one industrial dataset are largely transferable, and only minor low-rank adjustments are needed to adapt to a new domain.


\subsection{Anomaly Area vs.\ Detection and Localization Performance}
\label{sec:area}

We investigate how anomaly size affects detection and localization performance. Our initial hypothesis was that large anomalies would degrade INP extraction (fewer normal patches available for prototype aggregation), leading to worse detection. The results reveal a more nuanced picture.
...
The practical implication is twofold: (1) INP-Former's detection failure mode lies in small, subtle defects---precisely the regime where industrial inspection is most challenging; (2) for large defects, detection is reliable but localization quality drops, which may matter for downstream repair or root-cause analysis.
...
The model exhibits rapid early convergence: at epoch 25, I-AUROC already reaches 99.3\% and P-AUPRO reaches 94.6\%---within 0.4 points of the final value. The loss drops 68\% in the first 25 epochs (0.395$\to$0.163) but only 22\% over the remaining 175 epochs. From epoch 100 onward, improvements are marginal ($<$0.1 P-AUPRO points per 50 epochs). This suggests that 50--100 epochs may be sufficient for practical deployment, halving the training cost with $<$0.2\% metric degradation.


\subsection{AU-PRO at Strict FPR Threshold}
\label{sec:aupro_strict}

The standard AU-PRO metric integrates the per-region overlap curve up to FPR$\leq$0.3. We additionally evaluate at a stricter threshold of FPR$\leq$0.05, which better reflects industrial inspection requirements where false positives are costly (Table~\ref{tab:aupro_strict}).

\begin{table}[t]
\caption{AU-PRO at standard (FPR$\leq$0.3) vs.\ strict (FPR$\leq$0.05) thresholds.}
...
This is expected: the DINOv2 encoder is trained on natural images and can handle brightness variations, but color shifts alter the feature distribution more fundamentally. Per-category analysis reveals that categories with uniform textures (wallplugs, walnuts) are most sensitive to lighting shifts, while structurally distinct categories (vial) remain robust.
...
\paragraph{Lighting Robustness.} The lightshift analysis on MVTec AD 2 shows that INP-Former is robust to brightness changes but vulnerable to color shifts. Since the DINOv2 encoder is frozen, the model cannot adapt to out-of-distribution color statistics at test time. Data augmentation during training or test-time normalization could potentially mitigate this.
...
\paragraph{Limitations of This Study.} Due to computational constraints, we could not: (1) reproduce the full 5-configuration loss ablation on both MVTec AD and VisA as in the original paper, (2) run multi-seed experiments on VisA, or (3) compare against all baselines reported in the original paper. We focused our computational budget on the central claims and the most informative ablations.
...
We successfully reproduce the central claims of INP-Former~\citep{inp_former_2025}. The method achieves state-of-the-art multi-class anomaly detection on MVTec AD and VisA, matching the reported numbers within 0.1 percentage points. The INP mechanism and soft-mining loss are confirmed to be important components through our ablation study. Our extensions reveal that: (1) performance on the more challenging MVTec AD 2 is substantially lower, with different optimal hyperparameters; (2) LoRA-based transfer learning recovers 97--98\% of fully-trained performance on MVTec AD using only 5\% of trainable parameters; (3) the INP mechanism improves not only mean performance but also training stability across random seeds; (4) the model largely converges by epoch 50, with diminishing returns beyond epoch 100; (5) small anomalies ($<0.5\%$ area) are the primary failure mode, with detection rate dropping to 63.6\%; (6) pixel-level localization degrades substantially ($-$20.9 AU-PRO points) under strict FPR thresholds; and (7) color shifts in lighting degrade performance by up to 13.7 I-AUROC points, while brightness changes have minimal impact.
...
Wallplugs shows the most severe degradation: I-AUROC drops from 44.7\% to 25.3\% under shift\_3, and P-AUPRO from 64.8\% to 38.0\% under shift\_2. Walnuts is selectively vulnerable: shift\_1 collapses I-AUROC from 97.3\% to 47.3\% while shift\_2 has no effect. Vial remains the most robust category, maintaining $>$90\% I-AUROC across all conditions. Fruit\_jelly lacks shift\_2 and shift\_3 test sets in the dataset.
