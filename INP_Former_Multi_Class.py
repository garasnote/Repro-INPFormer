...
from dataset import MVTecDataset, MVTecAD2Dataset, RealIADDataset
...
def main(args):
    ...
    if args.dataset == 'MVTec-AD' or args.dataset == 'VisA':
        ...
        for i, item in enumerate(args.item_list):
            ...
            train_data.class_to_idx = {item: i}
            train_data.samples = [(sample[0], i) for sample in train_data.samples]
            test_data = MVTecAD2Dataset(root=item_path, transform=data_transform, gt_transform=gt_transform, phase="test")
            train_data_list.append(train_data)
            test_data_list.append(test_data)
        train_data = ConcatDataset(train_data_list)
        train_dataloader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=4, drop_last=True)
    ...
    model = INP_Former(encoder=encoder, bottleneck=Bottleneck, aggregation=INP_Extractor, decoder=INP_Guided_Decoder,
                             ...
                             fuse_layer_decoder=fuse_layer_decoder, prototype_token=INP, use_inp=not args.no_inp)
    ...
    if args.phase == 'train':
        ...
        for epoch in range(args.total_epochs):
            ...
            for img, _ in tqdm(train_dataloader, ncols=80):
                ...
                loss = global_cosine_hm_adaptive(en, de, y=args.loss_y)
                loss = loss + args.loss_lambda * g_loss
                optimizer.zero_grad()
                loss.backward()
            ...
            eval_at = set([args.total_epochs])
            if args.eval_epochs:
                eval_at.update(int(e) for e in args.eval_epochs.split(','))
            if (epoch + 1) in eval_at:
        ...
        # Test
        weight_path = args.load_from if args.load_from else os.path.join(args.save_dir, args.save_name, 'model.pth')
...
if __name__ == '__main__':
    ...
    # ablation: loss controls (defaults match paper)
    parser.add_argument('--loss_y', type=int, default=3, help='Soft mining exponent (0=disable soft mining)')
    parser.add_argument('--loss_lambda', type=float, default=0.2, help='Coherence loss weight (0=disable coherence loss)')

    parser.add_argument('--seed', type=int, default=1, help='Random seed')
    parser.add_argument('--no_inp', action='store_true', help='Disable INP mechanism (decoder uses self-attention on features instead)')
    parser.add_argument('--eval_epochs', type=str, default='', help='Comma-separated epochs to evaluate at (e.g. 25,50,100,150)')

    # cross-dataset testing: load weights from a different run
    parser.add_argument('--load_from', type=str, default=None, help='Path to model.pth (overrides save_dir/save_name)')

    args = parser.parse_args()
    inp_tag = 'noINP' if args.no_inp else 'INP'
    args.save_name = args.save_name + f'_dataset={args.dataset}_Encoder={args.encoder}_Resize={args.input_size}_Crop={args.crop_size}_INP_num={args.INP_num}_y={args.loss_y}_lambda={args.loss_lambda}_seed={args.seed}_{inp_tag}'
    ...
    if args.dataset == 'MVTec-AD':
        ...
        args.item_list = ['can', 'fruit_jelly', 'rice', 'sheet_metal', 'vial', 'wallplugs', 'walnuts']
    main(args)
