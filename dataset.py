...
class MVTecAD2Dataset(torch.utils.data.Dataset):
    ...
    def load_dataset(self):
        ...
        if self.phase == 'train':
            ...
            good_paths = glob.glob(os.path.join(self.img_path, 'good') + "/*.png")
            ...
            bad_paths = sorted(glob.glob(os.path.join(self.img_path, 'bad') + "/*.png"))
            gt_paths = sorted(glob.glob(self.gt_path + "/*.png"))
